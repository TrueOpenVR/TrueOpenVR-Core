...
- Для разработчиков приложений и игр: [демонстрационные примеры](https://github.com/TrueOpenVR/TrueOpenVR-Samples) и [документация](https://github.com/TrueOpenVR/TrueOpenVR-Core/tree/master/Docs).
...
- Поддержка VR шлемов (PlayStation VR, Oculus CV1, DK1, DK2, OSVR HDK и так далее).

- Поддержка трекеров (Aruco маркеры, Point трекер и так далее) для самодельных VR шлемов.

- Поддержка контроллеров (Razer Hydra, PS Move, Oculus Touch, Daydream, Gear VR), а также разработка и поддержка контроллера ("Arduino + светящиеся шарики для пинг понга + датчики вращения + веб-камера" or "Lighten Ping Pong Balls + Daydream контроллера + веб-камера") и "бумажного" контроллера ("веб-камера + Aruco маркеры + Arduino" или "веб-камера + Aruco маркеры + Daydream контроллера").
