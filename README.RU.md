...
- Поддержка VR шлемов (PlayStation VR, Oculus CV1, DK1, DK2, OSVR HDK, HTC Vive и так далее).

- Поддержка трекеров (Aruco маркеры, Point трекер и так далее) для самодельных VR шлемов.

- Поддержка контроллеров (Razer Hydra, PS Move, Oculus Touch, Nolo, HTC Vive, Daydream, Gear VR), а также разработка и поддержка "бумажного" контроллера ("веб-камера + Aruco маркеры + Arduino" или "веб-камера + Aruco маркеры + Daydream").
